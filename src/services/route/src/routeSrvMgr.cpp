    ...
    if(array)
    {
        ...
        if(rootJson)
        {
            ...
            if(gwFullData)
            {
                ...
                RDK_LOG( RDK_LOG_INFO, LOG_NMGR, "[%s:%s:%d] gateway count in json %d \n", MODULE_NAME,__FUNCTION__, __LINE__,gwCount);
        ...
        {
            retVal=FALSE;
            RDK_LOG( RDK_LOG_INFO, LOG_NMGR, "[%s:%s:%d] JSON is empty \n", MODULE_NAME,__FUNCTION__, __LINE__ );
...
gboolean RouteNetworkMgr::checkAddRouteInfo(char *ipAddr,bool isIPv4,char *ipv6Pfix)
{
    ...
    {
        RDK_LOG( RDK_LOG_INFO, LOG_NMGR, "[%s:%s:%d] Adding route since prefix changed from %s to %s \n", MODULE_NAME,__FUNCTION__, __LINE__,gwdata->ipv6Pfix->str,ipv6Pfix);
        lastRouteSetV4=isIPv4;
        addRouteToList(ipAddr,isIPv4,ipv6Pfix);
    }
    else
...
//            if((g_list_find(gwList,tempIP) == NULL))
                ...
                {
                    RDK_LOG( RDK_LOG_INFO, LOG_NMGR, "[%s:%s:%d] prefix changed from %s to %s \n", MODULE_NAME,__FUNCTION__, __LINE__,routeInfoData->ipv6Pfix->str,gwdata->ipv6prefix->str);
                    checkRemoveRouteInfo(routeInfoData->ipStr->str,routeInfoData->isIPv4);
                }
                else
                {
        ...
        {
            RDK_LOG( RDK_LOG_INFO, LOG_NMGR, "[%s:%s:%d] existing route are fine \n", MODULE_NAME,__FUNCTION__, __LINE__);
            return false;
        }
        RDK_LOG( RDK_LOG_TRACE1, LOG_NMGR, "[%s:%s:%d] Exit\n", MODULE_NAME,__FUNCTION__, __LINE__ );
        return true;
