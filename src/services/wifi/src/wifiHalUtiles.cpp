        ...
        if(RETURN_OK == wifi_disconnectEndpoint(1, wifiConnData.ssid))
        {
            ...
            while (start_time < timeout_period)
            {
                if(WIFI_DISCONNECTED == gWifiAdopterStatus)  {
                    ...
                    eventData.data.wifiStateChange.state = (ret)? WIFI_CONNECTING: WIFI_DISCONNECTED;
...
void *lafConnPrivThread(void* arg)
{
    ...
    {
        RDK_LOG(RDK_LOG_ERROR, LOG_NMGR, "[%s:%s:%d] wait for barrier_laf_connect failed\n",MODULE_NAME, __FUNCTION__, __LINE__ );
    }

    while (true ) {
        pthread_mutex_lock(&mutexLAF);
        while (triggerLAF == false)
        {
        ...
        while (gWifiLNFStatus != CONNECTED_PRIVATE)
        {
            ...
            {
                setLNFState(CONNECTED_PRIVATE);
                RDK_LOG( RDK_LOG_TRACE1, LOG_NMGR, "[%s:%s:%d] connection status %d is LAF current ssid %d \n", MODULE_NAME,__FUNCTION__, __LINE__,gWifiAdopterStatus,isLAFCurrConnectedssid );
                break;
            }
...
//        getDeviceActivationState();
    ...
    RDK_LOG( RDK_LOG_TRACE1, LOG_NMGR, "[%s:%s:%d] Exit\n",MODULE_NAME, __FUNCTION__, __LINE__ );
    pthread_barrier_destroy(&barrier_laf_connect);
    pthread_exit(NULL);
}
...
void connectToLAF()
{
    ...
    int ret = pthread_barrier_wait(&barrier_laf_connect);
    if (ret && ret != PTHREAD_BARRIER_SERIAL_THREAD)
    {
        RDK_LOG(RDK_LOG_ERROR, LOG_NMGR, "[%s:%d] wait for barrier_laf_connect failed\n", __FUNCTION__, __LINE__ );
    }
...
bool storeMfrWifiCredentials(void)
{
    ...
    {
        retVal=false;
        RDK_LOG(RDK_LOG_TRACE1,LOG_NMGR,"[%s:%s:%d] fetched ssid details \n",MODULE_NAME,__FUNCTION__,__LINE__ );
    }
...
bool getRadioStats(WiFi_Radio_Stats_Diag_Params *params)
{
    ...
    {
        RDK_LOG( RDK_LOG_TRACE1, LOG_NMGR, "[%s]Malloc Memory allocation failure\n", MODULE_NAME,__FUNCTION__);
        return false;
    }
