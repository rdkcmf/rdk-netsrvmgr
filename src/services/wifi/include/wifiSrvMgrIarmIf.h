...
typedef struct _WiFi_Radio_Diag_Params {
    ...
    char name[BUFF_LENGTH_24];
    unsigned int lastChange;
    char lowerLayers[BUFF_LENGTH_64];
    bool upstream;
    unsigned int maxBitRate;
    char supportedFrequencyBands[BUFF_LENGTH_24];
    char operatingFrequencyBand[BUFF_LENGTH_24];
    char supportedStandards[BUFF_LENGTH_24];
    char operatingStandards[BUFF_LENGTH_24];
    char possibleChannels[BUFF_LENGTH_256];
    char channelsInUse[BUFF_LENGTH_24];
    unsigned int channel;
    bool autoChannelSupported;
    bool autoChannelEnable;
    unsigned int autoChannelRefreshPeriod;
    char operatingChannelBandwidth[BUFF_LENGTH_24];
    char extensionChannel[BUFF_LENGTH_24];
    char guardInterval[BUFF_LENGTH_24];
    int mcs;
